...
def main():
    ...
    if (
        ...
        raise RuntimeError(
            "This builder supports only Lite without optional components"
        )
    ...
    manifest = {
        ...
        "upstream": {
            ...
            "asset": lock["asset"]["name"],
            "sha256": lock["asset"]["sha256"],
    ...
    subprocess.run(
        [
            str(ROOT / "target/release/sfx-pack"),
            str(ROOT / "target/release/sfx-stub"),
            str(stage),
            str(out),
            str(policy["max_sfx_bytes"]),
        ],
        check=True,
    )
